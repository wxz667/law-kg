...
from .types import ReferenceFilterContext, ReferenceFilterProfiling, ReferenceFilterResult
...
def run(
    ...
    context = build_reference_filter_context(graph_bundle)
    ...
    checkpoint_every = int(checkpoint_every or runtime.reference_filter_config().get("checkpoint_every", 50) or 50)
    accumulated_candidates: list[ReferenceCandidateRecord] = []
    accumulated_stats_counter: Counter[str] = Counter()
    accumulated_profiling = ReferenceFilterProfiling()
    ...
    def handle_document_result(_document_id: str, document_result: Any) -> None:
        ...
        with checkpoint_lock:
            ...
            snapshot_stats = build_reference_filter_stats(
    ...
    stats = build_reference_filter_stats(
    ...
    return ReferenceFilterResult(candidates=merged_candidates, stats=stats, profiling=profiling.to_dict())


def build_reference_filter_context(graph_bundle: GraphBundle) -> ReferenceFilterContext:
    context = build_reference_graph_context(graph_bundle)
    document_nodes = context.document_nodes
    return ReferenceFilterContext(
...
def merge_scan_results(
    results: list[Any],
) -> tuple[list[ReferenceCandidateRecord], Counter[str], ReferenceFilterProfiling]:
    profiling = ReferenceFilterProfiling()
...
def count_reference_filter_units(
    graph_bundle: GraphBundle,
    source_document_ids: set[str] | None = None,
) -> int:
    context = build_reference_filter_context(graph_bundle)
...
def build_reference_filter_stats(
    *,
