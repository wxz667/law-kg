...
def resolve_llm_conflict_runtime_config(runtime: Any, *, default_batch_size: int = 1) -> RelationConflictRuntimeConfig:
    classify_config = runtime.relation_classify_config()
    raw = dict(classify_config.get("llm_conflict", {}))
    provider = str(raw.get("provider", "")).strip()
    model = str(raw.get("model", "")).strip()
    if not provider or not model:
        raise ValueError("builder.relation_classify.llm_conflict must define non-empty provider and model.")
    ...
    if max_tokens < minimum_required_tokens:
        raise ValueError(
            "builder.relation_classify.llm_conflict params are inconsistent: "
...
def build_request_config(config: RelationConflictRuntimeConfig) -> ProviderRequestConfig:
    return ProviderRequestConfig(
        ...
        model=config.model,
        params={**config.params, "timeout_seconds": config.request_timeout_seconds, "max_retries": config.max_retries},
    )
