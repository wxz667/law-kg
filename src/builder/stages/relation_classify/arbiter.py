...
from ...contracts import LlmJudgeDetailRecord, RelationClassifyRecord
...
from .types import PendingArbitration, RelationClassifyResult
...
def arbitrate_uncertain_candidates(
    *,
    ...
    checkpoint_callback: Callable[[list[RelationClassifyRecord], list[LlmJudgeDetailRecord], int], None] | None = None,
) -> RelationClassifyResult:
    emit_progress(progress_callback, 0, len(pending))
    results_by_start: dict[int, list[RelationClassifyRecord]] = {}
...
def build_chunk_outputs(
    ...
    decisions: list[Any],
    stats: dict[str, Any],
) -> tuple[list[RelationClassifyRecord], list[LlmJudgeDetailRecord]]:
